...
# PINN 5D (t,x,y,x0,y0): Drift + Difusión + Trapping + Masa
# Geometría planar + campo uniforme + curriculum de pérdidas
# + early stopping + batería de 16 combinaciones de pérdidas (experimentos)
# + pi-test (Welch) y diagnósticos completos (electrón) para el modelo FULL.
#
# Adaptado para ejecución en GPU de la universidad:
#  - Forzamos backend CUDA y comprobamos nvidia-smi.
#  - Backend matplotlib "Agg" si no hay DISPLAY.
#  - Guardamos todas las figuras en ablation/figs/<RUN_TAG> (PNG + PDF).
#  - Guardamos resumen de métricas y pi-test en ficheros de texto/JSON.
...
# ========= 0.1) CUDA/JAX mínimo (GPU obligatoria) =========
...
if not _cmd_ok("nvidia-smi -L"):
    raise SystemExit("❌ GPU no visible. Ejecuta en un nodo GPU con CUDA.")

import jax
import jax.numpy as jnp
import jax.scipy.stats as js_stats  # (no imprescindible, pero lo dejamos)
...
# ========= 0.2) Gestión de figuras (guardar en disco y mostrar al final) =========
FIGS = []

def add_fig(fig):
    """Registrar una figura para guardarla al final."""
    FIGS.append(fig)

def save_fig(fig):
    """Guardar una figura en el buffer global (sin cerrar, para poder mostrarla)."""
    fig.tight_layout()
    add_fig(fig)
    # NO la cerramos aquí; se guarda + se podrá mostrar al final


def finalize_figures(figs_root: str = FIGS_ROOT, run_tag: str = RUN_TAG):
    """Guardar todas las figuras acumuladas en PNG + PDF y (si procede) mostrarlas."""
    ...
    print(f"[FIGS] {len(FIGS)} figuras guardadas en {os.path.abspath(outdir)}/ (PNG + all_figs.pdf)")

    # Mostrar todas las figuras si el backend no es Agg
    try:
...
def pde_residual_advective_5D(model, cfg: Dict, txyx0y0N: jnp.ndarray) -> jnp.ndarray:
    ...
    def residual_one(z):
        val = fN(z)
        g   = jax.jacfwd(fN)(z)          # grad wrt (tN,xN,yN,x0N,y0N)
...
def train_curriculum_5D(model, cfg_orig: dict, cfg_exp: dict, *,
    ...
    print("[CHECK] Modelo 5D: entrada=5 ⇒", getattr(static_model, "mlp").in_size if hasattr(static_model, "mlp") else "desconocido")
    ...
    for ep in range(1, epochs+1):
        ...
        # Early stopping basado en pde_RMS
        if pde_rms < best_metric - float(min_delta):
            ...
            if early_stopping and epochs_no_improve >= patience:
                print(f"[EARLY-STOP] ep={ep:4d} sin mejora en pde_RMS durante {patience} épocas "
                      f"(mejor={best_metric:.3e} en ep={best_epoch}).")
                break
    ...
    # Si usamos early stopping (Fase A), nos quedamos con el mejor modelo
    # según pde_RMS. Si NO hay early stopping (Fase B/C), usamos la última época.
    if early_stopping:
    ...
    logs["best_pde_rms"] = float(best_metric)   # sigue guardando el mejor pde_RMS
...
NORMALIZE_VIS = True  # normalizar cada frame al máximo local
...
def plot_map_electron_moving(
    ...
    """
    Mapa 2D para la evolución temporal del paquete de carga.
    - Colormap divergente (coolwarm).
    - Zona fuera de la caja original en gris.
    - Misma escala en x e y (círculo perfecto en todas las figuras).
    """
    Z = np.asarray(np.clip(Z, 0.0, 1.0))
    if normalize_max and Z.max() > 0:
        Z = Z / float(Z.max())
    ...
    # Transponer para ser coherentes con imshow
    Z_plot = Z.T
    mask_plot = (~mask_inside).T  # True = fuera ⇒ gris

    Z_masked = np.ma.array(Z_plot, mask=mask_plot)

    cmap = plt.get_cmap("coolwarm").copy()  # divergente
    ...
    im = ax.imshow(
        ...
        origin="lower",
        extent=extent,
        aspect="equal",          # <- misma escala en x e y
        vmin=0.0,
        vmax=1.0,
        cmap=cmap,
...
def widths_xy(Z: np.ndarray, X: np.ndarray, Y: np.ndarray, mask=None):
    """Anchuras σx, σy (en metros) como desviaciones típicas del paquete dentro de la caja."""
    Zp = np.clip(Z, 0.0, 1.0)
    if mask is not None:
        Zp = Zp * mask
    s = Zp.sum()
...
# === Figuras tipo “panel” y “redondas” (las podemos seguir usando si queremos extras) ===

def plot_mass_evolution_panels_for_seed(X, Y, Z_list, times, mask_orig,
                                        bounds_original, y_anode, x0_seed, y0_seed):
    """
    Panel de subplots con la evolución completa de la masa f(t,x,y;x0,y0)
    para una semilla (cada columna un instante).
    """
...
def plot_diffusion_circles_for_seed(X, Y, times, com_x, com_y, sigma_x, sigma_y,
                                    bounds_original, y_anode, x0_seed, y0_seed):
    """
    Figura tipo "redondas" que crecen:
      - Cada círculo está centrado en el centro de masa (COM_x, COM_y)
      - El radio es proporcional al ancho (σ_x, σ_y) en ese instante.
    """
    Xn = np.asarray(X); Yn = np.asarray(Y)
    ...
    # Rectángulo del detector original
    draw_original_box(ax, bounds_original)
    # Línea del ánodo
...
def plot_superposed_mass_trajectory(
    ...
    """
    Un único mapa con la suma de todas las instantáneas Z_snapshots:
    'masa arrastrándose' (todos los tiempos superpuestos).
    """
    if not Z_snapshots:
        return None, None
    ...
    fig_traj, ax_traj = plot_map_electron_moving(
        ...
        cfg_orig,
        title=title,
        normalize_max=False,  # ya normalizado
    )
...
def plot_com_path_with_diffusion(
    ...
    """
    Camino del COM en el plano (x,y) + círculos de difusión:
      - Línea que une las posiciones del centro de masa.
      - Un círculo en cada instante con radio ~ (σx+σy)/2.
      - Línea horizontal del ánodo.
    """
    com_x = np.asarray(com_x, dtype=float)
    com_y = np.asarray(com_y, dtype=float)
    sigma_x = np.asarray(sigma_x, dtype=float)
...
WIDTH, DEPTH = 256, 6
RAMP = 600          # rampa completa justo en la época 600
LR = 3e-4
LOG_EVERY = 100
...
# Etiquetas estándar de configuraciones
FULL_LABEL = "MASS+ANCHOR+BC+RING"
BEST_ABLATION_LABEL = "MASS+BC+RING"

# Semillas de evaluación (posiciones iniciales)
seeds_eval = [
    (1.5e-3, 2.5e-3),
    (0.6e-3, 4.4e-3),
    (2.4e-3, 3.6e-3),
...
# ==========================
# 8) Diagnósticos completos para el modelo FULL
# ==========================
...
def run_full_model_diagnostics(
    ...
    """
    Diagnósticos completos para un modelo entrenado (por defecto FULL):
      - Chequeos a t=0 frente a gaussiana teórica.
      - Mapas f(t,x,y;x0,y0) por tiempos (colormap divergente, fuera gris).
      - Paneles de evolución de la masa (todas las épocas) para las 4 seeds.
      - Contornos superpuestos de la trayectoria del paquete.
      - Figuras de difusión tipo “redondas” para las 4 seeds.
      - Trayectoria COM_y(t) y σ_x(t), σ_y(t) para cada seed.
      - Curvas de pérdidas de entrenamiento (total, IC, PDE, MASS, ANCHOR, BC, RING, PDE_RMS).
      - Decaimiento de masa vs exponencial teórica (seed 0) + COM/σ extra para seed 0.
      - Mapa con todos los tiempos superpuestos + camino del COM + círculos de difusión.
    """
    print(f"[EVAL {model_label}] Semillas (x0,y0):", seeds_eval)
    ...
    for (x0_seed, y0_seed) in seeds_eval:
        # ---- t = 0: comparación con gaussiana teórica ----
        ...
        # ---- Evolución temporal: snaps + mapas “electron moving” ----
        ...
        for tt in times:
            ...
            # Mapa tipo "electron moviéndose" (divergent + fuera gris)
        ...
        # ---- Trayectoria superpuesta: malla densa de tiempos para evitar "agujeros" ----
        if len(Z_snapshots) > 0:
            # Rejilla densa de tiempos entre 0 y el horizonte usado en el entrenamiento
            n_traj = 80  # puedes subir a 60–80 si quieres aún más continuidad
            t_min_traj = 0.0
        ...
        # Panel de evolución de masa (solo los tiempos "representativos")
        ...
        # Contornos superpuestos de la trayectoria (como en el código de referencia)
        if len(Z_snapshots) > 0:
        ...
        # Cálculo COM y σx, σy para las “redondas” y curvas COM/σ(t)
        ...
        # Figuras "redondas" (círculos que crecen)
        ...
        # Camino del COM + círculos de difusión (extra)
        ...
        # Curvas COM_y(t) y σ(t) para ESTA seed
        ...
        print(
            f"[{model_label}] ✅ El paquete cruza el ánodo."
            if crossed
            else f"[{model_label}] ⚠️ [WARN] El paquete no alcanzó el ánodo."
        )

    # ---- Curvas de pérdidas (logs de entrenamiento, como el código de referencia) ----
    if logs is not None:
        curves = [
            ("total",  "Total Loss"),
            ("ic",     "IC Loss"),
    ...
    # ---- Masa predicha vs masa esperada (exponencial) para la seed 0 ----
    tau_e = float(cfg_exp.get("tau_e", 0.0))
    if tau_e > 0.0:
        ...
        Q0 = Qs[0]
        # Normalizamos para ver directamente "predicted vs expected"
        ...
        # Trayectoria de COM y ensanchamiento (curvas) para la seed 0 (extra)
...
def welch_ttest(x, y):
    """
    Welch's t-test para dos muestras x, y.
    Devuelve (t_stat, p_value) con p-value bilateral.
    """
    ...
    # Si df no es finito, no podemos calcular p-value razonable
    if not np.isfinite(df):
        return float(t_stat), float("nan")

    # Intentar usar SciPy si está disponible
    try:
        from scipy import stats as scipy_stats
        p_val = float(2.0 * scipy_stats.t.sf(t_abs, df))
    except Exception:
        # Fallback: aproximación normal N(0,1)
        from math import erf, sqrt
        z = t_abs
        # p bilateral ≈ 2 * (1 - Phi(|z|)) = 1 - erf(|z| / sqrt(2))
        p_val = float(1.0 - erf(z / sqrt(2.0)))

    return float(t_stat), float(p_val)

# ==========================
# 10) Experimentos de pérdidas + pi-test
# ==========================
...
def run_experiment_suite(n_repeats: int = 2,
    ...
    """
    16 configuraciones (MASS, ANCHOR, BC, RING on/off),
    siempre con IC+PDE activas.
    Cada configuración se entrena n_repeats veces (semillas distintas)
    para poder hacer t-tests (pi-test) sobre pde_RMS.

    Si do_diagnostics=True, además se ejecutan los diagnósticos completos
    para el mejor modelo FULL (MASS+ANCHOR+BC+RING).
    Si do_diagnostics=False, solo se hacen los entrenamientos + pi-test.
    """
    ...
    for flags in combos:
        ...
        model_counter += 1
        print(f"\n===== MODELO {model_counter}/{total_models} :: Config={label}  extras={combo} =====")
    ...
    # Resumen tabla (en pantalla y luego lo guardaremos a fichero)
    ...
    # Pi-test: full vs full-sin-cada-término
    pi_tests = []
    if full_label not in metrics:
        print("[PI-TEST] Configuración completa no encontrada, se omite pi-test.")
    else:
        full_vals = np.array(metrics[full_label]["pde_rms"], dtype=float)
        print("\n[PI-TEST] Welch t-test (pde_RMS) full vs full-sin-cada-término")
        for extra in extras:
            ...
            if np.isnan(t_stat):
                print(f"  {extra}: no hay suficientes repeticiones para t-test.")
            else:
                print(
                    f"  {extra}: full ({full_label}) vs sin {extra} ({minus_label}) "
    ...
    # Diagnósticos completos para el mejor modelo FULL (solo si se pide)
    if do_diagnostics:
        if best_full["model"] is not None:
            print(
                "\n[DIAGNOSTICS] Usando el mejor modelo FULL (MASS+ANCHOR+BC+RING) "
                f"con pde_RMS={best_full['pde_rms']:.3e} para generar figuras y checks."
            )
            ...
            print("[DIAGNOSTICS] No se ha encontrado un modelo FULL entrenado correctamente.")
    else:
        print("\n[DIAGNOSTICS] Omitidos (Fase A: solo ablation + pi-test).")

    return metrics, best_full, pi_tests

# ==========================
# 11) Guardado de resultados numéricos
# ==========================
...
def save_experiment_results(metrics, best_full, pi_tests,
    ...
    """
    Guarda:
      - Tabla resumen de pde_RMS (mean ± std) en TXT.
      - Resultados de pi-test en TXT.
      - Métricas detalladas + pi-tests en JSON.
    Todo en la misma carpeta donde van las figuras: figs_root/run_tag/
    """
    import json
    ...
    with open(summary_txt, "w") as f:
        ...
        for row in pi_tests:
            f.write(
                f"  {row['extra']}: full ({row['full_label']}) vs sin {row['extra']} "
    ...
    print(f"[RESULTS] Resumen numérico guardado en:\n  {summary_txt}\n  {json_path}")

def label_to_combo(label: str):
    """
    Convierte un nombre de configuración como 'MASS+BC+RING' en un dict
...
def load_best_ablation_label(figs_root: str = FIGS_ROOT) -> Optional[str]:
    """
    Lee el último experimento de ablación (usando LAST_RUN.txt) y devuelve
    la etiqueta de la mejor configuración según la media de pde_RMS,
    excluyendo la FULL (MASS+ANCHOR+BC+RING).
    Si no se encuentra nada, devuelve None.

    (Esta función queda disponible por si se quiere seguir usando el criterio
    automático basado en la Fase A, pero la Fase B actual emplea por defecto
    la configuración fija BEST_ABLATION_LABEL = 'MASS+BC+RING'.)
    """
    import json

    last_path = os.path.join(figs_root, "LAST_RUN.txt")
    if not os.path.isfile(last_path):
        print("[PHASE B] WARNING: no LAST_RUN.txt encontrado; se omitirá el modelo de ablación.")
        return None
    ...
    if not os.path.isfile(json_path):
        print("[PHASE B] WARNING: no metrics_pde_rms_and_pitest.json en", last_dir)
        return None
    ...
    try:
        ...
        print(f"[PHASE B] WARNING: error leyendo {json_path}: {e}")
        return None

    metrics = data.get("metrics", {})
    if not metrics:
        print("[PHASE B] WARNING: fichero de métricas vacío; se omite el modelo de ablación.")
        return None

    best_label = None
    best_mean = None
    for label, m in metrics.items():
        # Saltamos el modelo FULL, que ya se entrena explícitamente en la Fase B
        if label == FULL_LABEL:
            continue
        vals = m.get("pde_rms", [])
        if not vals:
    ...
    if best_label is None:
        print("[PHASE B] WARNING: no se ha podido identificar un mejor modelo de ablación.")
    else:
        print(f"[PHASE B] Mejor config de ablación (último experimento) = {best_label} "
              f"(mean_pde_RMS ≈ {best_mean:.3e})")

    return best_label

# ==========================
# 12) Main: Fase A (ablation), Fase B (FULL+best ablation) y Fase C (tuning)
# ==========================

def main_phase_a():
    """
    FASE A: Ablation + pi-test.
    - Ajusta EPOCHS y RAMP solo para esta fase.
    - Entrena las 16 configuraciones con el mismo presupuesto de entrenamiento.
    - Hace Welch t-test (pi-test) SOLO con estos resultados.
    - No ejecuta diagnósticos físicos (se dejan para la Fase B/C).
    """
    global EPOCHS, RAMP

    # Presupuesto controlado para el experimento de ablación
    EPOCHS = 100     # todas las configuraciones, mismo número de épocas
    RAMP   = 80      # las pérdidas PDE/MASS rampan casi por completo dentro de esas 100

    print(f"[PHASE A] Ablation + pi-test con EPOCHS={EPOCHS}, RAMP={RAMP}")
    ...
    metrics, best_full, pi_tests = run_experiment_suite(
        ...
        do_diagnostics=False,   # aquí NO hacemos diagnósticos físicos
    )

    # Guardar resumen numérico (tabla + pi-test)
    ...
    # Guardar figuras (si hubiera; normalmente pocas o ninguna en Fase A)
    finalize_figures(FIGS_ROOT, RUN_TAG)


def main_phase_b():
    """
    FASE B: Comparación FULL vs mejor modelo de ablación.
    - Se entrena el modelo FULL (MASS+ANCHOR+BC+RING) con entrenamiento largo (EPOCHS=600).
    - Se entrena también el mejor modelo de la ablación escogido a priori: MASS+BC+RING.
      Esta elección es fija y no depende de que se haya ejecutado antes la Fase A.
    - Se ejecutan los mismos diagnósticos físicos para ambos modelos.
    - Al final se imprime cuál de los dos tiene mejor best_pde_RMS.
    """
    print(
        f"[PHASE B] Entrenando modelo FULL ({FULL_LABEL}) y modelo de ablación fijo "
        f"({BEST_ABLATION_LABEL}) con EPOCHS={EPOCHS}, RAMP={RAMP}"
    )

    # Geometría y dominio (común a ambos modelos)
    cfg_orig = load_geometry_from_dict(GEOMETRY)
    pad_px = int(GEOMETRY["expansion"]["pixels_per_side"])
    cfg_exp = prepare_phi_and_grad(expand_domain(cfg_orig, pad_px=pad_px))

    # Horizonte temporal estimado para cruzar el ánodo
    y_anode = cfg_orig["y_bounds"][0]
    vy = float(cfg_orig["vy"])
    y_center_guess = 0.5 * (cfg_orig["y_bounds"][0] + cfg_orig["y_bounds"][1])
    t_cross_center = (
    ...
    # 1) Modelo FULL
    # ==========================
    print("[PHASE B] Entrenando modelo FULL (MASS+ANCHOR+BC+RING)...")
    ...
    model_tr_full, logs_full, t_phys_max_full = train_curriculum_5D(
        ...
        ramp_epochs=RAMP, ema_beta=0.95, t_cross=t_cross_center, log_every=LOG_EVERY,
        sigma_ic=N0_SIGMA,
        early_stopping=False,              # <- 600 épocas sí o sí
        patience=EARLY_PATIENCE,
        min_delta=EARLY_MIN_DELTA,
    )
    ...
    print(
        ...
        f"en epoch={logs_full['best_epoch']}"
    )

    # Diagnósticos completos para FULL
    run_full_model_diagnostics(
    ...
    # ==========================
    # 2) Modelo de ablación fijo MASS+BC+RING
    # ==========================
    model_tr_ab = None
    logs_ab = None
    ...
    print(f"[PHASE B] Entrenando modelo de ablación '{best_ablation_label}' "
          f"con EPOCHS={EPOCHS}, RAMP={RAMP}...")
    ...
    model_tr_ab, logs_ab, t_phys_max_ab = train_curriculum_5D(
        ...
        ramp_epochs=RAMP, ema_beta=0.95, t_cross=t_cross_center, log_every=LOG_EVERY,
        sigma_ic=N0_SIGMA,
        early_stopping=False,              # <- también 600 épocas sí o sí
        patience=EARLY_PATIENCE,
        min_delta=EARLY_MIN_DELTA,
    )
    ...
    print(
        ...
        f"en epoch={logs_ab['best_epoch']}"
    )

    # Misma batería de diagnósticos para el mejor de ablación
    run_full_model_diagnostics(
    ...
    # ==========================
    # 3) Comparación final FULL vs modelo de ablación
    # ==========================
    full_val = float(logs_full["best_pde_rms"])
    abl_val  = float(logs_ab["best_pde_rms"])
    if abl_val < full_val:
        print(f"[PHASE B] RESULTADO: el modelo de ablación '{best_ablation_label}' "
              f"es mejor (pde_RMS={abl_val:.3e}) que FULL (pde_RMS={full_val:.3e}).")
    else:
        print(f"[PHASE B] RESULTADO: el modelo FULL sigue siendo mejor "
              f"(pde_RMS={full_val:.3e}) que '{best_ablation_label}' (pde_RMS={abl_val:.3e}).")

    # Guardar todas las figuras (FULL + ablación) en la misma carpeta
    finalize_figures(FIGS_ROOT, RUN_TAG)


def main_phase_c():
    """
    FASE C: Tuning de hiperparámetros para el mejor modelo de ablación (MASS+BC+RING).
    - Mantiene fija la combinación de pérdidas MASS+BC+RING.
    - Explora varias combinaciones de LR y ancho de la red.
    - Entrena cada configuración el mismo número de épocas.
    - Ejecuta diagnósticos físicos solo para la mejor configuración.
    """
    print(f"[PHASE C] Hyperparameter tuning para el modelo de ablación '{BEST_ABLATION_LABEL}'")
    ...
    if best_conf is not None:
        print("\n[PHASE C] Mejor configuración encontrada:")
        print(f"  lr={best_conf['lr']:.1e}, width={best_conf['width']} -> pde_RMS={best_pde:.3e}")

        # Diagnósticos para el mejor modelo ajustado
        run_full_model_diagnostics(
            best_model,
            cfg_orig,
            cfg_exp,
        ...
        print("[PHASE C] No se ha encontrado ninguna configuración válida de hiperparámetros.")

    finalize_figures(FIGS_ROOT, RUN_TAG)

def main_phase_d():
    """
    FASE D: MASS+BC+RING largo (1200 épocas, RAMP=800).
    Igual que la parte de ablación de la Fase B pero solo para MASS+BC+RING
    y con entrenamiento más largo.
    """
    # Hiperparámetros específicos de la fase D
    epochs_d = 1200
    ramp_d   = 800

    best_ablation_label = BEST_ABLATION_LABEL  # "MASS+BC+RING"
    print(f"[PHASE D] Entrenando modelo de ablación '{best_ablation_label}' "
          f"con EPOCHS={epochs_d}, RAMP={ramp_d}")

    # Geometría y dominio (igual que en Fase B)
    cfg_orig = load_geometry_from_dict(GEOMETRY)
    pad_px   = int(GEOMETRY["expansion"]["pixels_per_side"])
    cfg_exp  = prepare_phi_and_grad(expand_domain(cfg_orig, pad_px=pad_px))
    ...
    # Combinación MASS+BC+RING (igual que en B)
    combo       = label_to_combo(best_ablation_label)
    lam_mass    = LAM_MASS_BASE   if combo["MASS"]   else 0.0
    lam_anchor  = LAM_ANCHOR_BASE if combo["ANCHOR"] else 0.0
    lam_bc      = LAM_BC_BASE     if combo["BC"]     else 0.0
    ...
    model_tr_d, logs_d, t_phys_max_d = train_curriculum_5D(
        ...
        log_every=LOG_EVERY,
        sigma_ic=N0_SIGMA,
        early_stopping=False,          # <- 1200 épocas sí o sí
        patience=EARLY_PATIENCE,
        min_delta=EARLY_MIN_DELTA,
    )
    ...
    print(
        ...
        f"en epoch={logs_d['best_epoch']}"
    )

    # Diagnósticos físicos completos, igual que en Fase B
    run_full_model_diagnostics(
        model_tr_d,
        cfg_orig,
        cfg_exp,
    ...
    # Guardar todas las figuras de la fase D
    finalize_figures(FIGS_ROOT, RUN_TAG)

def main():
    """
    Selector de fase:
      - Fase A: experimento de ablación + pi-test (16 combos, épocas cortas).
      - Fase B: modelo FULL + mejor modelo de ablación fijo (MASS+BC+RING), entrenamiento largo + diagnósticos.
      - Fase C: tuning de hiperparámetros para el mejor modelo de ablación (MASS+BC+RING).
      - Fase D: MASS+BC+RING largo (1200 épocas, RAMP=800) con diagnósticos completos.

    Se elige con la variable de entorno PINN_PHASE:
      PINN_PHASE=A  -> Fase A
      PINN_PHASE=B  -> Fase B
      PINN_PHASE=C  -> Fase C
      PINN_PHASE=D  -> Fase D

    Si no se define PINN_PHASE, por defecto se ejecuta la Fase A.
    """
    ...
    if phase == "A":
        ...
        raise SystemExit(f"Valor de PINN_PHASE desconocido: '{phase}'. Usa 'A', 'B' o 'C' o D.")
