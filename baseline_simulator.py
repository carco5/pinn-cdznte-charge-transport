...
TITLE_FONTSIZE = 12           # smaller title so it doesn't collide with the colorbar
...
def plot_electron_frame(Z, X_ext, Y_ext, config, title=""):
    ...
    fig.colorbar(im, ax=ax, shrink=0.9, pad=0.04)
...
if __name__ == "__main__":
    ...
    for t in [0, 100, 200, 300, 400, 2_000, 10_000]:
        fig, ax = plot_electron_frame(
            ...
            Y_ext,
            config,
            title=f"Drift-only: electron density n (step={t}, t={t*dt:.2e} s)",
        )
