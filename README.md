...
The repository is intentionally minimal and contains four files:

- `baseline_simulator.py`  
  Drift-only finite-difference simulator on a 2D Cartesian grid.  
  It uses a prescribed electrostatic potential to compute an electric field and evolves a Gaussian electron packet under pure drift.  
...
- `pinn_5d_full_suite.py`  
  ...
  - Curriculum training for the composite loss (IC + PDE + MASS + BC + RING, plus optional anchor term).
...
- `requirements.txt`  
  List of Python packages needed to run the PINN code.
