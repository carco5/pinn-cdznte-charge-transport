...
It selects the training / evaluation workflow via the PINN_PHASE environment variable:
...
PINN_PHASE=C – Hyperparameter tuning of the best ablation model.
...
where <RUN_TAG> is generated automatically from the current date/time, and can be overridden via the PINN_RUN_TAG environment variable.
The root directory for figures can be changed with PINN_FIGS_DIR if needed.
